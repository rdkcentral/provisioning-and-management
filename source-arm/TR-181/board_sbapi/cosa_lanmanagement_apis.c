...
static int setLanIpv6ULAPrefix(char *ula_prefix) {
    ...
    if(CCSP_SUCCESS != PSM_Set_Record_Value2(bus_info, g_GetSubsystemPrefix(g_pDslhDmlAgent), PSM_LANMANAGEMENTENTRY_LAN_ULA_PREFIX, ccsp_string, ula_prefix) )
    {
        return ANSC_STATUS_FAILURE;
    }
...
static int setLanIpv6ULA(char *ula_prefix) {
    ...
    CCSP_MESSAGE_BUS_INFO *bus_info = (CCSP_MESSAGE_BUS_INFO *)bus_handle;
    if(CCSP_SUCCESS != PSM_Set_Record_Value2(bus_info, g_GetSubsystemPrefix(g_pDslhDmlAgent), PSM_LANMANAGEMENTENTRY_LAN_ULA, ccsp_string, ula_prefix) )
    {
        return ANSC_STATUS_FAILURE;
    }
...
static ANSC_STATUS setLanIpv6Enable(BOOLEAN ipv6_enable) {
    ...
    if(CCSP_SUCCESS != PSM_Set_Record_Value2(bus_info, g_GetSubsystemPrefix(g_pDslhDmlAgent), PSM_LANMANAGEMENTENTRY_LAN_IPV6_ENABLE, ccsp_string, ( ipv6_enable ) ? "TRUE"  : "FALSE"))
    {
        return ANSC_STATUS_FAILURE;
    }
...
static ANSC_STATUS setLanIpv6UlaEnable(BOOLEAN ula_enable) {
    ...
    if(CCSP_SUCCESS != PSM_Set_Record_Value2(bus_info, "eRT.", PSM_LANMANAGEMENTENTRY_LAN_ULA_ENABLE, ccsp_string, ( ula_enable ) ? "TRUE"  : "FALSE"))
    {
        return ANSC_STATUS_FAILURE;
    }
...
ANSC_STATUS CosaDmlLanManagement_SetLanIpv6Ula(char *lan_prefix, int lan_prefix_len, char *lan_ula, int lan_ula_len) {
    ...
    if((strlen(lan_prefix) == 0) || (strncmp(lan_prefix, "undefined", lan_prefix_len) == 0)) {
        ...
        if(CCSP_SUCCESS != PSM_Set_Record_Value2(bus_info, g_GetSubsystemPrefix(g_pDslhDmlAgent),
                       PSM_LANMANAGEMENTENTRY_LAN_ULA_PREFIX, ccsp_string, lan_prefix) ) {
            return ANSC_STATUS_FAILURE;
        }
        if(CCSP_SUCCESS != PSM_Set_Record_Value2(bus_info, g_GetSubsystemPrefix(g_pDslhDmlAgent),
                       PSM_LANMANAGEMENTENTRY_LAN_ULA, ccsp_string, lan_ula) ) {
            return ANSC_STATUS_FAILURE;
        }
    }
    ...
    )
{
    ...
    //Configure LAN ULA events wherever it is applicable
