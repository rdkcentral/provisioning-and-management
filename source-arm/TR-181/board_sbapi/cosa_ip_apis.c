...
static int _is_in_linux_bridge(char * if_name, char * br_name)
{
    ...
    bridge_get_info(br_name, &bridge);
    for (i = 0; i < bridge.slave_count; i++) {
        if (strstr(bridge.slave_name[i], if_name)) {
            ret = 1;
            break;
        }
    }
...
#endif
#endif
#if defined(_COSA_INTEL_USG_ARM_) && !defined(_ENABLE_DSL_SUPPORT_) && !defined(_RDKB_GLOBAL_PRODUCT_REQ_)
    ...
    )
{
    ...
    if (TRUE)
    {
        ...
        if (!Utopia_Init(&utctx))
        {
            AnscTraceWarning(("CosaDmlIpIfGetEntry: Utopia_Init failed\n"));
        }
    ...
    )
{
    ...
    else
    {
        ...
        if (strcmp(pCfg->Alias, p_be_buf_cfg->Alias) != 0)
        {
            ...
            if(!Utopia_Init(&utctx))
               AnscTraceWarning(("CosaDmlIpIfSetCfg: Utopia_Init failed\n"));
        ...
        if (pCfg->MaxMTUSize != p_be_buf_cfg->MaxMTUSize)
        {
            ...
            if (strstr(pCfg->LinkName, "erouter0"))
            {
                ...
                /*CID: 72276 Unchecked return value = Check Utopia_Set return value*/
                if(!Utopia_Set(&utctx, UtopiaValue_WAN_MTU, buf))
                {
                   AnscTraceWarning(("CosaDmlIpIfSetCfg: Utopia_Set failed for key UtopiaValue_WAN_MTU\n"));
                }
                Utopia_Free(&utctx, 1);
            }
            else
    ...
    )
{
    ...
    {    
        ...
        if (strcmp(pEntry->Alias, p_be_buf->Alias) != 0)
        {
            ...
            if(!Utopia_Init(&utctx))
               AnscTraceWarning(("CosaDmlIpIfSetV4Addr: Utopia_Init failed\n"));
...
int _datetime_to_secs(char * p_dt)
{
    ...
    /* Validate only the fields that have absolute constraints, not relative ones like year */
    if (t2.tm_mon < 1 || t2.tm_mon > 12 ||
    ...
    /* Check year is reasonable before converting to tm format */
    if (t2.tm_year < 1900)
    {
        CcspTraceWarning(("Year %d is before epoch, invalid\n", t2.tm_year));
        return 0;
    }
    ...
    /* mktime() will return -1 if the date is invalid or out of range */
    if (t1 == (time_t)-1)
    {
    ...
    /* Check if the difference fits in an int range */
    if (diff > (time_t)INT_MAX)
    {
        CcspTraceWarning(("Time difference %ld exceeds INT_MAX, clamping to INT_MAX\n", (long)diff));
        return INT_MAX;
    }
    if (diff < (time_t)INT_MIN)
    {
        CcspTraceWarning(("Time difference %ld below INT_MIN, clamping to INT_MIN\n", (long)diff));
        return INT_MIN;
    }
...
static int _invoke_ip_cmd(char * cmd, PCOSA_DML_IP_V6ADDR p_old, PCOSA_DML_IP_V6ADDR p_new, char * ifname, void * extra_args)
{
...
#ifdef CORE_NET_LIB
            addr_add_va_arg("-6 %s dev %s", buf, ifname);
#else
            v_secure_system("ip -6 addr add %s dev %s", buf, ifname);
#endif /* CORE_NET_LIB */
        else if (valid_lft && prefered_lft)
#ifdef CORE_NET_LIB
            addr_add_va_arg("-6 %s dev %s valid_lft %d preferred_lft %d", buf, ifname, valid_lft, prefered_lft);
#else
...
#ifdef CORE_NET_LIB
            addr_add_va_arg("-6 %s dev %s preferred_lft %d", buf, ifname, prefered_lft);
#else
    ...
    else if (!strcmp(cmd, "del"))
    {
        ...
        snprintf(buf+strlen(buf), sizeof(buf)-strlen(buf),  "/%d", pref_len);
#ifdef CORE_NET_LIB
        addr_delete_va_arg("-6 %s dev %s", buf, ifname);
#else
