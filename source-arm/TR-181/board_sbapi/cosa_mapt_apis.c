...
#ifdef CORE_NET_LIB
  route_delete_va_arg("default dev %s", MAPT_INTERFACE);
  if ( route_add_va_arg("default dev %s mtu %s", MAPT_INTERFACE , MAPT_V4_MTU_SIZE) )
#else
...
#ifdef CORE_NET_LIB
  libnet_status err = route_add_va_arg("-6 %s dev %s metric 256 mtu %s",
...
)
{
  ...
  if ( (fd = v_secure_popen("r","ip -4 route show | grep default | grep erouter0")) )
  {
       ...
       {
#ifdef CORE_NET_LIB
            route_delete("default dev erouter0");
            if ( route_add("default dev erouter0") )
#else
