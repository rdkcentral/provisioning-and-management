...
log "INFO" "Running make for CcspPandMSsp_gtest.bin..."
...
if [ -f "./source/test/CcspPandMSsp_gtest.bin" ]; then
    log "INFO" "Running CcspPandMSsp_gtest.bin"
    ./source/test/CcspPandMSsp_gtest.bin
    log "INFO" "Completed Test Execution"
else
    log "ERROR" "CcspPandMSsp_gtest.bin does not exist, cannot run tests"
    exit 1
fi
