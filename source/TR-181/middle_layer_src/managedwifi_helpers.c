	    ...
	    msgpack_object obj = msg.data;
                ...
                (MSGPACK_OBJECT_MAP == msg.data.type) )
            {
                ...
                if( NULL != wrapper && 0 != strncmp(wrapper,"Parameters",strlen("Parameters"))) 
                {
                    ...
                    if (inner == NULL) {
                        CcspTraceWarning(("Wrapper message pack is not proper\n"));
			errno = MW_HELPERS_MISSING_WRAPPER;
		    }
		    else if((0 == (process)(p, 1, inner)) || ((true == optional) && (NULL == inner)) )
                    {
		    ...
		    if(NULL == inner || NULL == subdoc_name || NULL == version || NULL == transaction_id) {
                        CcspTraceWarning(("Wrapper message pack is not proper\n"));
			errno = MW_HELPERS_MISSING_WRAPPER;
		    }
		    else if( (0 == (process)(p,4, inner, subdoc_name, version, transaction_id)) || ((true == optional) && (NULL == inner)) )
                    {
                         msgpack_unpacked_destroy( &msg );
                         errno = MW_HELPERS_OK;
                         return p;
