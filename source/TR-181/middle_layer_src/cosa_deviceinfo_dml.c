    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "Enable", TRUE))
    {
        ...
        // When Enable is set to false, clean up cron jobs
        if (bValue == FALSE)
        {
			v_secure_system("crontab -l 2>/dev/null | sed '/rdkbLogMonitor.sh/d' | crontab -");
            v_secure_system("crontab -l 2>/dev/null | sed '/fileUploadRandom.sh/d' | crontab -");
