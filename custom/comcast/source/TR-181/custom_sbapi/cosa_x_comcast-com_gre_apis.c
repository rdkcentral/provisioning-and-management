...
CosaDml_GreTunnelInit(void)
{
    ...
    /** In XER10 product, decide to create Hotspot tunnel circuit thread based on Hotspot feature
     ** available. Else we are getting excessive logging in devices in case if feature is not
     * available.
     */
    char buf[64]      = {0};
    sysevent_get(sysevent_fd, sysevent_token, "HotSpotSupport", buf, sizeof(buf));
    if (buf[0] != '\0') {
        if (strncmp (buf, "false", strlen("false")) == 0) {
            CcspTraceInfo(( "%s %d Hotspot feature disabled for platform, so no need to create Hotspot GRE Tunnel init thread \n", __FUNCTION__, __LINE__ ));
